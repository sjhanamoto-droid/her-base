...
const ChipRow: React.FC<{ chips: Chip[] }> = ({ chips }) => (
  <div className="mt-8 flex flex-wrap gap-3">
    {chips.map((c, i) => (
      <div key={i} className="flex items-center gap-2 border border-ink/15 px-4 py-2 text-xs md:text-sm text-ink-600">
    ...
    <section id="pillars" className="relative isolate overflow-hidden py-24 md:py-32 border-b border-ink/10 bg-gradient-to-b from-base-200 via-base-100 to-base-50">
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
        ...
        <div
          ...
          style={{ background: 'radial-gradient(circle at center, rgba(200,161,123,0.45), rgba(200,161,123,0) 68%)', animation: 'pillarsOrbA 22s ease-in-out infinite' }}
        ></div>
        <div
          className="absolute -bottom-40 -right-24 w-[42rem] h-[42rem] rounded-full blur-3xl"
          style={{ background: 'radial-gradient(circle at center, rgba(173,134,96,0.32), rgba(173,134,96,0) 68%)', animation: 'pillarsOrbB 27s ease-in-out infinite' }}
        ></div>
        <div
          className="absolute top-1/2 left-1/2 w-[30rem] h-[30rem] rounded-full blur-3xl"
          style={{ background: 'radial-gradient(circle at center, rgba(200,161,123,0.22), rgba(200,161,123,0) 70%)', animation: 'pillarsOrbC 19s ease-in-out infinite' }}
        ></div>
        ...
        <div
          ...
          style={{
            backgroundImage:
              'linear-gradient(rgba(42,32,22,0.55) 1px, transparent 1px), linear-gradient(90deg, rgba(42,32,22,0.55) 1px, transparent 1px)',
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 md:px-12">
        <AnimatedSection direction="up" delay={0.1}>
          <div className="text-center mb-16 md:mb-20">
            <p className="font-display tracking-[0.35em] text-brand-700 text-sm mb-5 uppercase">Two Axes</p>
            <h2 className="font-serif text-3xl md:text-4xl font-semibold text-ink leading-relaxed tracking-[0.06em]">
              あなたの人生を、2つの軸でサポート
            </h2>
            <span className="block w-16 h-px mx-auto mt-7 bg-gradient-to-r from-transparent via-brand-500 to-transparent"></span>
          </div>
        </AnimatedSection>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
          ...
          <AnimatedSection direction="right" delay={0.2}>
            <div className="h-full bg-base-50/70 backdrop-blur-md border border-ink/15 p-8 md:p-12 shadow-[0_20px_60px_-28px_rgba(42,32,22,0.35)] transition-transform duration-500 hover:-translate-y-1">
              <div className="flex items-center gap-5 mb-7">
                <span className="flex-shrink-0 w-14 h-14 border border-ink/15 flex items-center justify-center text-brand-500">
                  <HeartHandshake className="w-6 h-6" strokeWidth={1.5} />
                </span>
                <div>
                  <p className="font-display tracking-[0.2em] text-brand-700 text-xs md:text-sm uppercase">HERBASE Mindset</p>
                  <h3 className="font-serif text-xl md:text-2xl font-semibold text-ink mt-1.5">支えるから、一緒に挑戦するへ。</h3>
                </div>
              </div>
              <p className="text-ink-600 leading-[2] text-[0.95rem] md:text-base">
          ...
          <AnimatedSection direction="left" delay={0.3}>
            <div className="h-full bg-base-50/70 backdrop-blur-md border border-ink/15 p-8 md:p-12 shadow-[0_20px_60px_-28px_rgba(42,32,22,0.35)] transition-transform duration-500 hover:-translate-y-1">
              <div className="flex items-center gap-5 mb-7">
                ...
                <div>
                  <p className="font-display tracking-[0.2em] text-brand-700 text-xs md:text-sm uppercase">HERBASE Life Design</p>
